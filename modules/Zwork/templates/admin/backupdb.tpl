...
<div class="z-admin-content-pagetitle">
    ...
    <h3>{gt text='Backup database'}</h3>
</div>

<form class="z-form" action="{modurl modname="Zwork" type="admin" func="executebackup"}" method="post" enctype="application/x-www-form-urlencoded">
    <input type="hidden" name="csrftoken" value="{insert name="csrftoken"}" />
    <fieldset>
        <legend>{gt text='Backups'}</legend>
        <div class="z-informationmsg">
            {gt text='Here is list of past database backups, available in directory:'} {$vars.zwork_backupsdir|safetext}<br />
        ...
        <div class="z-formrow">
            ...
            <select id="past_backup" name="past_backup" size="10">
                ...
                <option value="{$index}">{$backup}</option>
                {/foreach}
            </select>
        </div>
        <div class="z-formrow">
            <label for="backupsdir">{gt text='Backups directory'}</label>
            <input id="backupsdir" type="text" name="backupsdir" value="{$vars.zwork_backupsdir|safetext}" disabled />
        </div>
    </fieldset>
    <div class="z-buttons z-formbuttons">
        {button src="button_ok.png" set="icons/extrasmall" __alt="Create backup" __title="Create backup" __text="Create backup"}
